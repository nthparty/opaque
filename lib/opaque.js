module.exports = function (io, sodium, oprf) {
  ...
  // const test = function () {
  //   ps = oprf.generateRandomScalar();
  //   Ps = sodium.crypto_scalarmult_ristretto255_base(ps);
  //
  //   pu = oprf.generateRandomScalar();
  //   Pu = sodium.crypto_scalarmult_ristretto255_base(pu);
  //
  //   xs = oprf.generateRandomScalar();
  //   Xs = sodium.crypto_scalarmult_ristretto255_base(xs);
  //
  //   xu = oprf.generateRandomScalar();
  //   Xu = sodium.crypto_scalarmult_ristretto255_base(xu);
  //
  //   util.KE(ps, xs, Pu, Xu, Xs);
  //   util.KE(pu, xu, Ps, Xs, Xu);
  // };
  //
  // return {client_register:test};
  ...
  const server_register = function (op_id) {
    op_id = op_id + ':pake_init';
    var get = io.get.bind(null, op_id);
    var give = io.give.bind(null, op_id);
  ...
  // Try to log in
  const client_authenticate = function (password, user_id, op_id) {
    op_id = op_id + ':pake';
    var get = io.get.bind(null, op_id);
    var give = io.give.bind(null, op_id);
    ...
    return new Promise(function (resolve) {
      ...
      const pw = util.oprf_KDF(util.oprf_KDF(password));
      ...
      get('beta').then(function (b) {
        if (sodium.crypto_core_ristretto255_is_valid_point(b)) {
          get('c').then(function (c) {
            ...
            const rw = util.oprf_H(util.oprf_raise(b, r_inv), mask);
            ...
            get('Xs').then(function (Xs) {
              ...
              // const ssid1 = H(sid, ssid, a);
              const SK = util.oprf_F(K, util.sodium_from_byte(0));//, ssid1);
              const As = util.oprf_F(K, util.sodium_from_byte(1));//, ssid1);
              const Au = util.oprf_F(K, util.sodium_from_byte(2));//, ssid1);
              ...
              get('As').then(function (__As) {
                if (sodium.compare(As, __As) === 0) {  // The comparable value of 0 means As equals __As
                  ...
                  give('client_authenticated', false);  // maybe give server a heads up
                  resolve(false);
                }
              });
            });
