module.exports = (io, sodium, oprf) => {
  ...
  const clientRegister = (password, user_id, op_id) => {
  ...
  // Try to log in
  const clientAuthenticate = (password, user_id, t, op_id) => {
    op_id = op_id + ':pake';
    const get = io.get.bind(null, op_id);
    const give = io.give.bind(null, op_id);
              ...
              get('Xs').then((Xs) => {
                ...
                get('As').then((__As) => {
                  if (sodium.compare(As, __As) === 0) {  // The comparable value of 0 means As equals __As
                    give('Au', Au);
                    get('authenticated').then((success) => {
                      if (success) {
  ...
  // Authenticate a user
  const serverAuthenticate = (user_id, pepper, op_id) => {
    op_id = op_id + ':pake';
    const get = io.get.bind(null, op_id);
    const give = io.give.bind(null, op_id);
